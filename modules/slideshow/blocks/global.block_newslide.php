...
if (!nv_function_exists('nv_silide_blocks')) {
    ...
    function nv_silide_blocks($block_config) {
        global $module_array_cat, $module_info, $lang_module, $site_mods, $db, $global_config;
        ...
        while ($rows = $rs -> fetch()) {
            $array_th[] = array('id' => $rows['id'], 'name' => $rows['name'], 'url' => $rows['url'], 'link' => NV_BASE_SITEURL . NV_UPLOADS_DIR . "/" . $module . "/" . $rows['link']);
        }

        if (file_exists(NV_ROOTDIR . "/themes/" . $global_config['site_theme'] . "/modules/" . $module_file . "/block_newslide.tpl")) {
            $block_theme = $global_config['site_theme'];
        } else {
            $block_theme = "default";
        }

        $xtpl = new XTemplate("global.block_newslide.tpl", NV_ROOTDIR . "/themes/" . $block_theme . "/modules/" . $block_config['module']);
