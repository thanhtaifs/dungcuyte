...
if( ! function_exists( 'nv_pro_catalogs' ) )
{
	...
	function nv_pro_catalogs( $block_config )
	{
        ...
        //file_put_contents(NV_ROOTDIR . '/module_check.log', $module . PHP_EOL, FILE_APPEND);
		...
		foreach ($array_cat_shops  as $cat) {
            if ($cat['parentid'] == 0) {
                $has_sub = !empty($cat['subcatid']) ? true : false;
                $class = $has_sub ? ' class="has-submenu"' : '';
	...
	function html_viewsub($list_sub, $block_config)
    {
        ...
        foreach ($list as $catid) {
            if (!isset($array_cat_shops[$catid])) {
                continue;
            }
    
            $cat = $array_cat_shops[$catid];
            $has_sub = !empty($cat['subcatid']) ? true : false;
            $class = $has_sub ? ' class="has-submenu"' : '';
