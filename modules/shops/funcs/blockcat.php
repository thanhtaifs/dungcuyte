...
if (isset($array_op[1])) {
    ...
    // ✅ Đã sửa phần sắp xếp block theo thứ tự ABC
    $stmt = $db->prepare('SELECT bid, ' . NV_LANG_DATA . '_title, ' . NV_LANG_DATA . '_alias, image, ' . NV_LANG_DATA . '_description, ' . NV_LANG_DATA . '_keywords FROM ' . $db_config['prefix'] . '_' . $module_data . '_block_cat ORDER BY ' . NV_LANG_DATA . '_title COLLATE utf8_vietnamese_ci ASC');
