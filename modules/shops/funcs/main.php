...
//file_put_contents(NV_ROOTDIR . '/menu_debug.log', "Run: home_view = " . $pro_config['home_view'] . "\n", FILE_APPEND);


if( empty( $contents ) )
{
    //file_put_contents(NV_ROOTDIR . '/menu_debug.log', "Run if empty content \n", FILE_APPEND);
	$data_content = array();
	$html_pages = '';
	$orderby = '';
	if( $sorts == 0 )
	...
	if( $pro_config['home_view'] == 'view_home_blocks' )
	{
        ...
        $result_featured = $db->query( $db->sql() );
        //file_put_contents(NV_ROOTDIR . '/menu_debug.log', "result_featured: " . $result_featured->rowCount() . "\n", FILE_APPEND);
        ...
        // Lấy 24 sản phẩm mới nhất (JOIN với bảng shops_block, bid = 3, không trùng với sản phẩm nổi bật)
        $where_new = 't1.inhome=1 AND t1.status =1 AND t2.bid = 3';
        if( !empty($featured_ids) )
        {
            $where_new .= ' AND t1.id NOT IN (' . implode(',', $featured_ids) . ')';
        }
        ...
        // Lấy các sản phẩm còn lại (với phân trang)
        $excluded_ids = array_merge($featured_ids, $new_ids);
        $where_other = 't1.inhome=1 AND t1.status =1';
        if( !empty($excluded_ids) )
        {
            $where_other .= ' AND t1.id NOT IN (' . implode(',', $excluded_ids) . ')';
        }
        
        // Đếm số sản phẩm còn lại cho phân trang
        ...
        if( empty( $other_products ) and $page > 1 )
        {
...
//file_put_contents(NV_ROOTDIR . '/menu_debug.log', "Chưa vào empty content \n", FILE_APPEND);
if( $page > 1 )
{
	$page_title .= ' ' . NV_TITLEBAR_DEFIS . ' ' . $lang_global['page'] . ' ' . $page;
	$description .= ' ' . $page;
}
//file_put_contents(NV_ROOTDIR . '/menu_debug.log', "page: " . $page. "\n", FILE_APPEND);
include NV_ROOTDIR . '/includes/header.php';
