...
/**
 * thanhtaifs dev Edit home block
 *
 * @param mixed $data_content
 * @param string $html_pages
 * @return
...
function view_home_blocks($data_content, $compare_id, $html_pages, $sorts) 
{
    ...
    // Kiểm tra xem data_content có phải là mảng phân loại không
    $is_categorized = false;
    if (!empty($data_content) && is_array($data_content)) {
        if (isset($data_content['featured']) || isset($data_content['new']) || isset($data_content['other'])) {
            $is_categorized = true;
        }
    }
    ...
    if ($is_categorized) 
    {
        ...
        if (!empty($data_content['featured'])) {
            ...
            $xtpl->parse('main.featured_section');
        }
...
function display_product_item(&$xtpl, $data_row, $num_row, $pro_config, $global_array_shops_cat, $global_array_group, $compare_id, $array_wishlist_id, $lang_module, $section = 'items')
{
    ...
    $xtpl->assign('SRC_PRO_LAGE', $data_row['homeimgfile']);
...
function view_home_all($data_content, $compare_id, $html_pages = '', $sort = 0, $viewtype = '')
{
    ...
    //file_put_contents(NV_ROOTDIR . '/menu_debug.log', "Vào view_home_all method \n", FILE_APPEND);
    ...
    if (!empty($data_content)) {
        //file_put_contents(NV_ROOTDIR . '/menu_debug.log', "Vào if content \n", FILE_APPEND);
        $i = 1;
        $num_row = 24 / $pro_config['per_row'];
        //file_put_contents(NV_ROOTDIR . '/menu_debug.log', "Run pro_config  = " . $pro_config['per_row'] . "\n", FILE_APPEND);
        if ($op == 'main') {
            $xtpl->parse('main.new_product_title');
        }
        ...
        foreach ($data_content as $data_row) {
            ...
            //file_put_contents(NV_ROOTDIR . '/menu_debug.log', "SP ID: " . $data_row['id'] . " | Giá: " . $data_row['product_price'] . "\n", FILE_APPEND);
            ...
            if (isset($data_row['product_type'])) {
                if ($data_row['product_type'] == 'featured') {
                    $xtpl->parse('main.items.featured_badge');
                } elseif ($data_row['product_type'] == 'new') {
                    $xtpl->parse('main.items.new_badge');
                }
            }
...
function viewcat_page_gird($data_content, $compare_id, $pages, $sort = 0, $viewtype = '')
{
   ...
   //file_put_contents(NV_ROOTDIR . '/debug_seek.log', ": pro_config" . $pro_config['show_displays'] . "\n", FILE_APPEND);
    ...
    //file_put_contents(NV_ROOTDIR . '/debug_seek.log', "per_row:" . $pro_config['per_row'] . "\n", FILE_APPEND);
    if (!empty($data_content['data'])) {
        ...
        foreach ($data_content['data'] as $data_row) {
            ...
            $xtpl->assign('variant_label', empty($data_row['variant_label']) ? '' : $data_row['variant_label']);
    ...
    if (!empty($data_content)) {
        ...
        if (!$popup) {
            ...
            if (!empty($data_others)) {
                ...
                //file_put_contents(NV_ROOTDIR . '/nv_log.txt', print_r($hmtl, true), FILE_APPEND);
...
function cart_product($data_content, $coupons_code, $order_info, $array_error_number, $total_cart = '', $coupon_data = array())
{
    ...
    if (!empty($data_content)) {
        ...
        foreach ($data_content as $data_row) {
            $xtpl->assign('stt', $j);
            $xtpl->assign('id', $data_row['id']);
            $xtpl->assign('cart_id', $data_row['cart_id']);
            $xtpl->assign('title_pro', $data_row['title']);
            $xtpl->assign('link_pro', $data_row['link_pro']);
            $xtpl->assign('img_pro', $data_row['homeimgthumb']);
            if (!empty($data_row['selected_price'])) {
            ...
            if (!empty($data_row['variant_label'])) {
                $xtpl->assign('variant_label', $data_row['variant_label']);
                $xtpl->parse('main.rows.variant');
            }
            // Tinh diem tich luy
            ...
            $price_unit = !empty($data_row['selected_price']) ? array('discount_percent' => 0) : nv_get_price($data_row['id'], $pro_config['money_unit'], $data_row['num'], true);
