...
function nv_page_main($row, $ab_links, $content_comment)
{
    ...
    //$xtpl = new XTemplate(options: 'main.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    //$xtpl = new XTemplate('main.tpl', NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file);
    try {
        $template_path = NV_ROOTDIR . '/themes/' . $module_info['template'] . '/modules/' . $module_file;
        ...
        $xtpl = new XTemplate($template_file, $template_path);
        //error_log('✅ XTemplate object created successfully');
    } catch (Throwable $e) 
    {
        error_log('❗ Exception while creating XTemplate: ' . $e->getMessage());
    }
    //error_log('policies module - XTemplate');
    ...
    if (defined('NV_IS_MODADMIN')) {
        if (defined('NV_CHECK_SESSION')) {
            $xtpl->assign('ADMIN_CHECKSS', md5($row['id'] . NV_CHECK_SESSION));
        } else {
            $xtpl->assign('ADMIN_CHECKSS', '');
            //error_log('⚠️ NV_CHECK_SESSION not defined in this context');
        }
        //$xtpl->assign('ADMIN_CHECKSS', md5($row['id'] . NV_CHECK_SESSION));
