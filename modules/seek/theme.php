...
function search_main_theme($is_search, $search, $array_modul)
{
    ...
    if ($is_search) {
        if ($search['is_error']) {
            $search_result = '<div class="alert alert-danger">' . $search['errorInfo'] . '</div>';
        } else {
            $search_result = !empty($search['content']) ? $search['content'] : '<div class="alert alert-warning">Không tìm thấy kết quả nào phù hợp với từ khóa <strong>' . htmlspecialchars($search['key']) . '</strong>.</div>';
        }
    }
    $xtpl->assign('SEARCH_RESULT', $search_result);
...
function search_result_theme($result_array, $mod, $mod_custom_title, $search, $is_generate_page, $limit, $num_items)
{
    ...
    foreach ($result_array as $result) 
    {
        ...
        {
            $result['thumbnail_image'] = NV_BASE_SITEURL . 'themes/default/images/no-image.jpg';
        }
       
        //file_put_contents(NV_ROOTDIR . '/debug_seek.log', 'thumbnail: ' .  $result['thumbnail'] . PHP_EOL, FILE_APPEND);
