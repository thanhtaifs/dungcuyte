		...
		foreach( $blocks as $_row )
		{
			...
			if( in_array( $_row['position'], $array_position ) and nv_user_in_groups( $_row['groups_view'] ) )
			{
				...
				if( ! empty( $content ) or defined( 'NV_IS_DRAG_BLOCK' ) )
				{
					...
					elseif( file_exists( NV_ROOTDIR . '/themes/dungcuytecantho/layout/block.' . $_row['template'] . '.tpl' ) )
					{
						$xtpl = new XTemplate( 'block.' . $_row['template'] . '.tpl', NV_ROOTDIR . '/themes/dungcuytecantho/layout' );
					}
					if( ! empty( $xtpl ) )
					{
