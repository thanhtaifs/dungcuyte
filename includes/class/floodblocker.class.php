...
class FloodBlocker
{
	...
	private $rules = array(
		...
		60 => 300, // rule 2 - maximum 30 requests in 60 secs
		300 => 500, // rule 3 - maximum 50 requests in 300 secs
		3600 => 2000 // rule 4 - maximum 200 requests in 3600 secs
	);
	...
	private function CheckFlood()
	{
		...
		foreach( $this->rules as $interval => $limit )
		{
			...
			if( $info[$interval]['count'] > $limit )
			{
			    ...
			    // Reset lại bộ đếm để sau 3s không bị block tiếp
                $info[$interval]['count'] = 0;
                $info[$interval]['time']  = NV_CURRENTTIME;
    
                // Lưu log ngay để đảm bảo lần sau check không dính lại
